...
const DialogItem = observer(({dialogs}) => {
  let lastMessageText = "Пока что нет сообщений"; 

    if (dialogs.messages && dialogs.messages.length > 0) {
        // Извлечение текста последнего сообщения
        const lastMessage = dialogs.messages[0];
        lastMessageText = lastMessage.text;
    }

  console.log(dialogs , 'qweq')
    ...
    return (
      ...
      <div className={styles.BlockOfOneNew}  onClick={()=> navigate(DIALOGS_ROUTE + '/' + dialogs.friendInDialog.id)}>
          <div className={styles.ImageAndTextBlock}>
             ...
             ?<img className={styles.image} src={process.env.REACT_APP_API_URL + dialogs.friendInDialog.image }/>
             :<div className={styles.fakeImgBlock}></div> 
             }
             </div>
             <div className={styles.textInDialogsVariantsBlock}>
              ...
              <div className={styles.LastTextBlock}>
                <p className={styles.LastText}>{lastMessageText}</p>
              </div>
               </div>
